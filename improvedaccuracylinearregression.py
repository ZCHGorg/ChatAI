        ...
        if len(self.foreground_accuracy_history) >= num_iterations_for_change:
            ...
            if self.last_foreground_average_accuracy is not None:
                if last_n_avg_accuracy > self.last_foreground_average_accuracy:
                    change_color = "green"
                    change_verbiage = "Improving"
                    self.accuracy_change_count += 1
                elif last_n_avg_accuracy < self.last_foreground_average_accuracy:
                    change_color = "red"
                    change_verbiage = "Declining"
                    self.accuracy_change_count += 1
                else:
                    change_color = "blue"
                    change_verbiage = "No Change"

                bot_color = self.generate_unique_color(bot_name)

                avg_accuracy_colored = colored(f"{last_n_avg_accuracy:.4f}", "white", "on_grey")
                change_text_colored = colored(change_verbiage, change_color, attrs=["bold"])
                bot_name_colored = colored(bot_name, bot_color, attrs=["bold"])

                print(f"{bot_name_colored}, AVG.Accuracy: {avg_accuracy_colored}, Change: {change_text_colored}, Count: {self.accuracy_change_count}")
